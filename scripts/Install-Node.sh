...
if [ "$EUID" -ne 0 ]
        ...
        exit
else LINKTONODE=$(curl -sG https://nodejs.org/dist/latest-v8.x/ | awk '{print $2}' | grep -P 'href=\"node-v8\.\d{1,}\.\d{1,}-linux-'$PICHIP'\.tar\.gz' | sed 's/href="//' | sed 's/<\/a>//' | sed 's/">.*//');
# curl -G https://nodejs.org/dist/latest-v8.x/ | awk '{print $2}' | grep -P 'href=\"node-v8\.\d{1,}\.\d{1,}-linux-armv10l\.tar\.gz' | sed 's/href="//' | sed 's/<\/a>//' | sed 's/">.*//'
